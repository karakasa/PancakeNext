...
public sealed class QuantityArithmetic : ArithmeticRepository
{
    ...
    public static Quantity Abs(Quantity a)
        => a.IsNegative ? a.DuplicateAndNegate() : a;
    ...
    public static Quantity Truncate(Quantity a)
    {
        ...
        {
            return GetWholeAndRemainderPart(fi).Whole;
        }
    ...
    public static Quantity Remainder(Quantity a)
    {
        ...
        {
            return GetWholeAndRemainderPart(fi).Remainder;
        }

        throw new NotSupportedException();
    }

    private static (FeetInchLength Whole, FeetInchLength Remainder) GetWholeAndRemainderPart(FeetInchLength fi)
    {
        var wholePart = new FeetInchLength(fi.FeetIntegerPart, fi.InchIntegerPart, precision: fi.Precision);
        var remainder = new FeetInchLength(0, 0, fi.InchFractionPartFirst, fi.InchFractionPartSecond, precision: fi.Precision);
        if (fi.IsNegative)
    ...
    public static Quantity Floor(Quantity a)
    {
        ...
        {
            var (whole, remainder) = GetWholeAndRemainderPart(fi);
